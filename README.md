...
### 1. BNK-Project (학원 메인 프로젝트)
> **소개:** BNK 금융 서비스를 벤치마킹한 웹 뱅킹 서비스의 통합 관리자(Back-office) 시스템
> **개발 기간:** 2025.06 ~ 2025.08 (팀 프로젝트)

금융 상품의 라이프사이클을 관리하는 관리자 포털을 전담하여 개발했습니다. 특히 임시 테이블과 실 테이블을 분리하여 데이터 무결성을 보장하는 승인(인가) 프로세스를 설계하고 구현하는 데 주력했습니다.

- **Tech Stack**: `Java`, `Spring (STS4)`, `OracleDB`, `MyBatis`, `JSP`
- **My Role**:
  - **인가 결재 시스템**:상품 등록/수정 시 즉시 반영되지 않고, 상위 관리자의 승인을 거쳐 배포되는 로직 구현 (임시/실제 테이블 분리)
  - **통계 대시보드**: 검색어 로그 데이터를 수집하여 인기 검색어 분석 및 차트(Chart.js) 시각화
  - **권한 관리**: 상위/하위 관리자 등급(Role)에 따른 메뉴 접근 권한 제어 및 세션 관리
...
개발 공부를 본격적으로 시작하기 전, 대학 졸업 작품으로 만들었던 결과물입니다. 기획부터 완성까지 하나의 사이클을 경험해 보았습니다.

- **Tech Stack**: [사용 기술: HTML/CSS/JS 또는 Flutter 등]
- **My Role**:
  - 전체적인 디자인 및 화면 레이아웃 구성
  - 아이디어 기획 및 기능 구현
...
## Education
- **[학원 이름]** (202X.XX ~ 202X.XX)
  - 자바(Java) 기반 백엔드 개발자 양성 과정 수료
- **[대학교 이름]** (202X.XX ~ 202X.XX)
  - [전공학과명] 졸업
...
---
![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=smallomg&show_icons=true&theme=radical)
